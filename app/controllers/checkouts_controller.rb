class CheckoutsController < Spree::BaseController 
  ...
  update do
    ...
    success.wants.html do  
      if @order.checkout_complete # test for state change too?
        if current_user
        ...
        flash[:notice] = "Unexpected error condition -- please contact site owner"
    ...
    failure.wants.html do
      flash[:notice] = "Shouldn't occur -- contact site maintainer"
      redirect_to edit_object_url and next
    end
    failure.wants.js do   
      render :json => "ouch"
